...
Alice RAG utilizes [LlamaIndex](https://www.llamaindex.ai/) for ingesting documents and creating a vector store. The embedding model used is [`BAAI/bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5). To keep the repository lightweight, datasets and embeddings are fetched and generated during the Docker image build process.
...
Top passages retrieved from the vector database are then combined with the user’s query to generate a prompt for the selected Gemini model. The prompt follows guidelines from the [Gemini documentation](https://model.baai.ac.cn/model-detail/100112#usage) and a detailed article on [RAG architecture](https://medium.com/@saurabhgssingh/understanding-rag-building-a-rag-system-from-scratch-with-gemini-api-b11ad9fc1bf7). The format is as follows:
...
2. Use the following command to run the Docker container locally, exposing the application on port 8501, and setting the `GEMINI_API_TOKEN` environment variable.
...
The container is also available on DockerHub (for amd64 and arm64) at [pmiron/alice-rag-llm](https://hub.docker.com/repository/docker/pmiron/alice-rag-llm/general). You can pull the image from the repository and run it directly on your local machine.
